...
/**
 * Class OverrideTranslationsFactory
 * @package \PrestaShopBundle\Translation\Factory
 */
class OverrideTranslationsFactory implements TranslationsFactoryInterface
{
	...
	public function createTranslationsArray($domainIdentifier, $locale = self::DEFAULT_LOCALE, $theme = null, $search = null)
	{
		foreach ($this->providers as $provider) {
			if ($domainIdentifier === $provider->getIdentifier()) {
				if ('override' === $domainIdentifier) {
					/** @var \PrestaShopBundle\Translation\Provider\OverrideProvider $provider */
					$provider->setLocale($locale)->synchronizeOverride();
				}
